...
from schemas import models, schemas, company

default_session = AsyncSession(engine)


class CompanyCRUD(BaseCRUD):
    def __init__(self, session=default_session, model=models.Company, schema=company.Company):
    ...
    async def add(self, company: company.Company) -> Optional[models.Company]:
